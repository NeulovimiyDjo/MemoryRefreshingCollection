parser grammar PostgreSQLParser_bad;


options { tokenVocab = PostgreSQLLexer_bad; language = CSharp; }
@header
{
using System.Linq;
}
@members
{
	    ...
	    public static PostgreSQLParser_bad getPostgreSQLParser(string script)
	    {
	       var CharStream = CharStreams.fromString(script);
	       var Lexer = new PostgreSQLLexer_bad(CharStream);
	       var Tokens = new CommonTokenStream(Lexer);
	       var Parser = new PostgreSQLParser_bad(Tokens);
        ...
        {
            internal PostgreSQLParser_bad grammar;
...
createfunctionstmt
   : CREATE opt_or_replace (FUNCTION | PROCEDURE) func_name func_args_with_defaults
     ...
     createfunc_opt_list opt_routine_body
   ;
...
opt_routine_body
    : return_stmt
    | BEGIN_P routine_body_stmt_list END_P
    | /*EMPTY*/
    ;

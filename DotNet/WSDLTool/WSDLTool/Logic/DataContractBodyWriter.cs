...
{
    internal class DataContractBodyWriter : BodyWriter
    {
        ...
        public DataContractBodyWriter(ServiceDescription service, Binding binding) : base(isBuffered: true)
        {
            _service = service;
            _binding = binding;
        ...
        private void DiscoverTypes(Type type, bool isRootType)
        {
            ...
            foreach (var member in type.GetPropertyOrFieldMembers().Where(mbr =>
                        mbr.DeclaringType == type
                        && mbr.CustomAttributes.All(attr => attr.AttributeType.Name != "IgnoreDataMemberAttribute")
                        && !mbr.GetPropertyOrFieldType().IsPrimitive
                        && !SysTypeDic.ContainsKey(mbr.GetPropertyOrFieldType().FullName)
                        && mbr.GetPropertyOrFieldType() != typeof(ValueType)
                        && mbr.GetPropertyOrFieldType() != typeof(DateTimeOffset)
                        && mbr.GetPropertyOrFieldType().FullName != "System.Runtime.Serialization.ExtensionDataObject"))
            {
                Type propertyType;
                ...
                {
                    propertyType = member.GetPropertyOrFieldType().IsArray
                ...
                {
                    propertyType = member.GetPropertyOrFieldType();
                }
        ...
        private void WriteComplexType(XmlDictionaryWriter writer, Type type)
        {
            ...
            {
                var members = type.GetPropertyOrFieldMembers().Where(fld =>
                    fld.DeclaringType == type
                    && fld.CustomAttributes.All(attr => attr.AttributeType.Name != "IgnoreDataMemberAttribute")
                    && fld.GetPropertyOrFieldType().FullName != "System.Runtime.Serialization.ExtensionDataObject");

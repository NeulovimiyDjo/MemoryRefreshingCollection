...
namespace Cert_requester
{
  class Program
  {
    ...
    static async void GetChallengeNewAcc()
    {
      ...
      var account = await acme.NewAccount("mymailuser@mail.ru", true);
      var pemKey = acme.AccountKey.ToPem();
      File.WriteAllText("pem_key.txt", pemKey);

      var order = await acme.NewOrder(new[] { "mysite-001-site1.dtempurl.com" });
    ...
    static async void GenerateCert()
    {
      ...
      var cert = await order.Generate(new CsrInfo
      {
        ...
        CommonName = "mysite-001-site1.dtempurl.com",
      }, privateKey);

...
function DockerBuild([string]$imageName, [string]$imageTag) {
    if (((docker image ls -q "${imageName}:${imageTag}") | Measure-Object).Count -eq 0) {
        ...
        $null = $processRunner.ExecuteProcess("docker", "build --target ${imageName} --tag ${imageName}:${imageTag} --file ./build/Dockerfile-bases --build-arg BUILDKIT_INLINE_CACHE=1 . --progress=$Progress", 0)
    } else {
